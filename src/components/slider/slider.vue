<template>
  <div class='slider-container' :class = 's_data.containerClass' @mouseleave="swipeOut">
    ...
    <span v-if="s_data.pagination" class="slider-pagination slider-pagination-bullets">
        <div v-bind:key="n" v-for="n in (pagenums||s_data.sliderLength)" >
...
export default {
  ...
  computed: {
    prevPage: function () { if (this.data.currentPage > 0) { return this.$parent.someList[this.data.currentPage - 1].uri } else { return this.$parent.someList[this.s_data.sliderLength - 1].uri } },
    nextPage: function () { if (this.data.currentPage === (this.s_data.sliderLength - 1) || this.data.currentPage === this.s_data.sliderLength) { return this.$parent.someList[0].uri } else { return this.$parent.someList[this.data.currentPage + 1].uri } },
    styleobj: function () {
      let style = {}
    ...
    pagenums: function () {
      if ((this.pages.length || this.s_data.sliderLength !== 0) && !this.s_data.pageInit) {
          this.s_data.pageInit = true /* eslint-disable-line */
        this.$nextTick(() => {
  ...
  methods: {
    ...
    slide (pagenum, type) {
      this.data.prevPage = this.prevPage
      this.data.nextPage = this.nextPage
